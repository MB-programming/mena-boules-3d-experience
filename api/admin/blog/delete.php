...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && 'delete' === 'index') {
    ...
    } elseif ($_SERVER['REQUEST_METHOD'] === 'GET' && 'delete' === 'get') {
        if (!isset($_GET['id'])) {
        ...
        echo json_encode($result);
    } elseif ($_SERVER['REQUEST_METHOD'] === 'POST' && 'delete' === 'create') {
        $input = json_decode(file_get_contents('php://input'), true);
        $result = $blog->create($input, $admin['id']);
        http_response_code($result['success'] ? 201 : 400);
        echo json_encode($result);
    } elseif (($_SERVER['REQUEST_METHOD'] === 'PUT' || $_SERVER['REQUEST_METHOD'] === 'POST') && 'delete' === 'update') {
        $input = json_decode(file_get_contents('php://input'), true);
        if (!isset($input['id'])) {
            http_response_code(400);
            echo json_encode(['success' => false, 'message' => 'معرف المقالة مطلوب']);
        ...
        echo json_encode($result);
    } elseif (($_SERVER['REQUEST_METHOD'] === 'DELETE' || $_SERVER['REQUEST_METHOD'] === 'POST') && 'delete' === 'delete') {
        $input = json_decode(file_get_contents('php://input'), true);
        if (!isset($input['id'])) {
            http_response_code(400);
            echo json_encode(['success' => false, 'message' => 'معرف المقالة مطلوب']);
